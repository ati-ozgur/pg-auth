...
var functionSql = function() {
    ...
    _.each(files, function(file) {
        if (file.indexOf(".sql") > 0) {
            var sql = fs.readFileSync(functionDir + file, {
                encoding: "utf-8"
            });
            sqlFiles.push(sql);
        }
...
var tableSql = function() {
    ...
    _.each(files, function(file) {
        if (file.indexOf(".sql") > 0) {
            var sql =
                sqlFiles.push(fs.readFileSync(tableDir + file, {
                    encoding: "utf-8"
                }));
        }
    });
    ...
    var fks = fs.readFileSync(indexDir + "foreign_keys.sql", {
        encoding: "utf-8"
    });
    sqlFiles.push(fks);
...
exports.build = function() {
    ...
    //grab the init file
    var initSql = fs.readFileSync(srcDir + "init.sql", {
        encoding: "utf-8"
    });
...
exports.install = function() {
    ...
    massive.connect({
        ...
        var sql = self.build();
        if (!db || !db.run) {
            console.log("Unable to connect to database using connectionString'" + connectionString + "'.");
